...
def load_image_as_gray_tensor(path: str | os.PathLike[str], device: torch.device, dtype: torch.dtype) -> torch.Tensor:
    image = Image.open(path).convert("L")
    # 先转灰度，再按 MiniT2I-B/16 的默认输入规格中心裁剪并缩放到 512x512。
    image = center_crop_square(image).resize((TARGET_IMAGE_SIZE, TARGET_IMAGE_SIZE), Image.BICUBIC)
    # MiniT2I 的图像分支仍然期望 3 通道输入；这里把灰度图复制成 RGB 三通道。
